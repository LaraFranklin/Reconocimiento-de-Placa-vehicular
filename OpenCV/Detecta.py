...
def identificarPlaca(img, capturas):
    ...
    for c in contours:
        ...
        approx = cv2.approxPolyDP(c, 0.6 * peri, True)  # Approximating with 6% error
    ...
    cv2.imwrite("captura-%d.jpg" %capturas, new_image)
...
while True:
    ...
    capturas += 1
    cv2.imwrite('capturas/imagen-%d.jpg' %capturas, img)
    cv2.imshow('test',img)
